...
__attribute__((hot)) TrStr GpuGenerator_emit(GpuGenerator* self, HirProgram* prog, TrStr target) {
    ...
    if ((strcmp(_tr_strz(tgt), _tr_strz(_tr_str_lit(""))) == 0)) {
        /* pass */
        TrStr _strtmp_t3438 = _tr_str_lit("spirv");
        _tr_str_release(tgt);
        tgt = _strtmp_t3438;
    }
    /* pass */
    ...
    while ((de < dev_fns->len)) {
        ...
        e0->dev_fns = dev_fns;
        /* pass */
        ({ TrStr _sbt_t3439 = (GpuEmitter_emit_device_fn(e0, ((HirFunction*)List_ptr_get(dev_fns, de)))); StringBuilder_append(sb, _sbt_t3439); _tr_str_release(_sbt_t3439); });
        /* pass */
        if ((!e0->ok)) {
            /* pass */
            self->ok = false;
    ...
    while ((fi < prog->functions->len)) {
        ...
        if (fn_is_kernel(f)) {
            ...
            e->dev_fns = dev_fns;
            /* pass */
            ({ TrStr _sbt_t3440 = (GpuEmitter_emit_kernel(e, f)); StringBuilder_append(sb, _sbt_t3440); _tr_str_release(_sbt_t3440); });
            /* pass */
            if ((!e->ok)) {
                /* pass */
                self->ok = false;
    ...
    if (((strcmp(_tr_strz(tgt), _tr_strz(_tr_str_lit("nvptx"))) == 0) && (self->n_kernels > 0LL))) {
        ...
        while ((fi2 < prog->functions->len)) {
            ...
            if (fn_is_kernel(f2)) {
                ...
                /* pass */
                ({ TrStr _sbt_t3441 = (({ TrStr _cr = (_tr_str_wrap(_tr_int_to_str((long long)(ki)))); TrStr _cres = _tr_strx_concat(_tr_strz(_tr_str_lit("!")), _cr.data); _tr_str_release(_cr); _cres; })); StringBuilder_append(sb, _sbt_t3441); _tr_str_release(_sbt_t3441); });
                /* pass */
                ki = (ki + 1LL);
            }
            /* pass */
